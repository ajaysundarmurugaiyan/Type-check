...
// Split a passage into small sub-paragraphs of roughly 3-4 lines each, breaking
// at sentence ends (falling back to word boundaries) so each chunk reads
// naturally. The chunks concatenate back to the exact original text.
export function splitIntoChunks(text, targetLen = 300) {
  const chunks = [];
  let start = 0;
  const n = text.length;

  while (start < n) {
    let end = Math.min(n, start + targetLen);

    if (end < n) {
      const windowEnd = Math.min(n, start + Math.floor(targetLen * 1.5));
      let breakAt = -1;

      // Prefer a sentence end at/after the target length.
      for (let i = end; i < windowEnd; i++) {
        if (text[i] === "." && (text[i + 1] === " " || i + 1 === n)) {
          breakAt = i + 1;
          break;
        }
      }
      // Otherwise a sentence end a bit before the target.
      if (breakAt === -1) {
        for (let i = end; i > start + Math.floor(targetLen * 0.5); i--) {
          if (text[i] === "." && text[i + 1] === " ") {
            breakAt = i + 1;
            break;
          }
      ...
      // Otherwise break on the nearest space so we never split a word.
      if (breakAt === -1) {
        let i = end;
        while (i > start && text[i] !== " ") i--;
        breakAt = i > start ? i : end;
      }
      end = breakAt;
    }
...
// Sum per-chunk scores into one overall result. Each chunk is validated on its
// own (so a slip never leaks across paragraphs), then the totals are combined.
export function aggregateStats(chunks, typedChunks, timeUsedSec) {
  let correct = 0;
  let wrong = 0;
  let left = 0;
  let extra = 0;
  ...
  chunks.forEach((chunk, i) => {
    const s = computeStats(chunk, typedChunks[i] || "", 1);
    correct += s.correct;
    wrong += s.wrong;
    left += s.left;
    extra += s.extra;
